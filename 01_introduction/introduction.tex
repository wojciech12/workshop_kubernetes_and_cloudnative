...
We have prepared two simple application in Python and Golang, your task is to deploy them on the Kubernetes cluster. Please create an user on \href{https://hub.docker.com}{hub.docker.com}:
...
Create, following the instruction of the trainer, the following resources:

\begin{markdown}
1. Deployment with DB_USERNAME as an env variable
2. ConfigMap with a field DB_NAME
3. Secret
3. Service
4. Ingress, path: `/hello`
\end{markdown}

%
\section{Quick look on the configuration}
We will cover it in a separate workshop (that covers {\small CI}/{\small CD}), here is a quick start.%
...
Let's see each of these popular approaches. 

\bigskip
1. envsubst or similar approach. Let's imagine, we want to generate ingress resources with differenet host names:
...
\bigskip
3. Helm - use just what you need. If possible move configMaps to the component repo. Let's go through \texttt{manifests/helm/deployment}:

\begin{verbatim}
$ cd manifest/helm/deployment
...
3. It is OK, if you create Kubernetes secrets manually in the beggining. 
\end{markdown}

\section{Next}

\begin{itemize}
\item Liveness/Readiness probes - \href{https://github.com/wojciech12/talk_zero_downtime_deployment_with_kubernetes}{https://github.com/wojciech12/talk\_zero\_downtime\_deployment\_with\_kubernetes}
